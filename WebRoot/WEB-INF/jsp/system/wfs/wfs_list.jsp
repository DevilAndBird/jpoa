...
<div id="page-content" class="clearfix">				
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="wfs/golistpage" method="post" name="sheetForm" id="sheetForm">
			<table>
				<tr>
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="workflowname" placeholder="这里输入流程名单" />
							<i id="nav-search-icon" class="icon-search"></i>
						</span>
					</td>
					<td>
					        <input autocomplete="off" id="nav-search-input" type="text" name="sheetname" placeholder="这里输入表单名称" />
				    </td>
				<td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增表单";
			 diag.URL = '<%=basePath%>wfs/gosave.do';
			 diag.Width = 230;
			 diag.Height = 200;
		...
		function editUser(user_id){
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '<%=basePath%>wfs/goupdate.do?id='+user_id;
			 diag.Width = 230;
			 diag.Height = 200;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function delUser(userId,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>wfs/godelete.do?id="+userId;
