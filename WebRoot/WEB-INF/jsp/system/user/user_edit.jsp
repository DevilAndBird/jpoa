...
<script type="text/javascript">
	...
	function save(){
		...
		if(provid == -1 || !provid) {
			$("#seleProvid_modal").tips({
				side:3,
	            msg:'省份信息不为空',
	            bg:'#AE81FF',
	            time:2
	        });
			$("#seleProvid_modal").focus();
			return false;
		}
		
		if(cityid == -1 || !cityid) {
			$("#seleCityid_modal").tips({
				side:3,
	            msg:'市级信息不为空',
	            bg:'#AE81FF',
	            time:2
	        });
			$("#seleCityid_modal").focus();
			return false;
		}
...
<body>
	<form action="user/${msg }.do" name="userForm" id="userForm" method="post">
		...
		<table>
			...
			<tr>
				<td><tqm:select id="province" name="postid" postname="${pd.postid}"></tqm:select></td>
			</tr>
			<tr>
				<td>
					<select name="provid_modal" id="seleProvid_modal" data-placeholder="省份" style="vertical-align:top;width: 100px;">
						<option value="-1" selected="selected">--省份--</option>
					</select>
					<select name="cityid_modal" id="seleCityid_modal" data-placeholder="城市" style="vertical-align:top;width: 100px;">
