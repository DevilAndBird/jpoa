...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="prov/provList.do" method="post" name="Form" id="Form">
			<input type="hidden" name="id" id="id"/>
			<input type="hidden" name="isvalidV" id="isvalidV"/>
			<table>
				<tr>
					<td>
						<input autocomplete="off" type="text" name="name" id="name" value="${pd.name }" placeholder="省份名称" />
					</td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<!-- 开始循环 -->	
				<c:choose>
					<c:when test="${not empty provList}">
						<c:forEach items="${provList}" var="obj" varStatus="vs">
							<tr>
								...
								<td class="center" style="width: 120px;">
									<a title="删除" class="btn btn-mini btn-danger" onclick="delProv(${obj.id});" ><i class='icon-trash'></i></a>
									<a title="编辑" class='btn btn-mini btn-info updateProv'><i class='icon-edit'></i></a>
								</td>
							</tr>
						</c:forEach>
					</c:when>
		...
		<table style="width:100%;">
			<tr>
				<td style="vertical-align:top;"><a class="btn btn-small btn-success" id="provAdd">新增省份</a></td>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" style="width:538px;height:308px;display: none;">
	<div class="widget-box" style="width:540px;margin-left: -1px;margin-top: -2px;">
		...
		<div class="widget-body">
		 <div class="widget-main">
			...
			<hr />
			<div class="row-fluid">
				<label for="form-field-12">省份名称</label>
				<input type="text" name="modelName" id="modelName" value="" placeholder="安徽"/> 
			</div>
			<hr />
			<div class="row-fluid">
				<label for="form-field-10">状态</label>
				<!-- <input type="text" name="modelIsvalid" id="modelIsvalid" value="" placeholder="1、开启    2、禁用"/>  -->
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		$("#saveBtn").click(function(){
			...
			var url = "<%=basePath%>prov/insertProv.do";
			$("#Form").attr("action", url );
			$("#Form").submit();
		});
		...
		$(".updateProv").click(function(){
			...
			var isvalid = s.eq(2).text();
			if( isvalid.indexOf("开启")>0  ){
			...
			$.ajax({
				...
				success:function(flag){
			       if(flag==1){
			    	   ...
			    	   toastr.error('省份名称已经存在');
			    	   return ;
			       }else if(flag==2){
			    	   location.href="./prov/provList.do";
				   }
			    }
