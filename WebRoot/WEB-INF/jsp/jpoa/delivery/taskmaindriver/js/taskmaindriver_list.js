...
function updateTask(param) {
	var url = "./taskmaindriver/updateStatus.do?id="+$(param).val() + "&statusTemp=" + TASKMAINDRIVER_STATUS.CANCEL.value;
...
$("#btnSaveTask").click(function(){
	...
	var srcaddr = $("#seleSrcaddr_modal").attr("selected",true).val();
	var destaddr = $("#seleDestaddr_modal").attr("selected",true).val();
	...
	if(srcaddr == destaddr){
		toastr.warning('同一个集散中心无须设置运输人员!');
		return ;
	}
	
	$.ajax({
		url : "./taskmaindriver/insertTask.do",
		dataType:"text",
		type : "POST",
		data:{
			provid : provid,
		...
		success:function(flag){
			layer.alert(flag, {
				...
				,
				yes : function() {
					  location.href="./taskmaindriver/findTaskMainDriverlistPage";
				}
			});
	    }
