...
public class LoginController extends BaseController {
	...
	public Object login()throws Exception{
		...
		//Log lg = new Log();
//		String createdtime = DateUtil.getTime();
		//lg.setCreatedtime(createdtime);
		
		if(null != KEYDATA && KEYDATA.length == 3){
			//shiro管理的session
			Subject currentUser = SecurityUtils.getSubject();  
			Session session = currentUser.getSession();
			session.setTimeout(1000*60*60*12);// session超时时间
			...
			if(null == code || "".equals(code)){
				...
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){
					...
					pd = userService.getUserByNameAndPwd(pd);//de41b7fb99201d8334c23c014db35ecd92df81bc
					if(pd != null){
						...
						user.setProvid(pd.getString("provid"));
						user.setCityid(pd.getString("cityid"));
						String centralpointgps = areaCityService.findCentralpointgpsByid(pd.getString("cityid"));
						user.setBaiduCoord( (BaiduCoord) JSONObject.toBean(JSONObject.fromObject(centralpointgps), BaiduCoord.class));
						user.setPHONE(pd.getString("PHONE"));
						
						session.setAttribute(Const.SESSION_USER, user);
						session.removeAttribute(Const.SESSION_SECURITY_CODE);
					//	lg.setUsername(pd.getString("USERNAME"));
						//lg.setIp(pd.getString("IP"));
						//shiro加入身份验证
						...
						errInfo = "usererror"; 	
						//lg.setContent(errInfo);//用户名或密码有误
					}
				}else{
					errInfo = "codeerror";	
					//lg.setContent(errInfo);//验证码输入有误
				}
				if(Tools.isEmpty(errInfo)){
					errInfo = "success";	
					//lg.setContent(errInfo);//验证成功
				}
			}
		}else{
			errInfo = "error";
			//lg.setContent(errInfo);//缺少参数
		}
		//logeservice.save(lg);
