...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="road/roadList.do" method="post" name="Form" id="Form">
			<input type="hidden" name="id" id="id"/>
			<input type="hidden" name="isvalidV" id="isvalidV"/>
			<table>
				<tr>
					<td style="vertical-align:top;"> 
					 	<select class="chzn-select" name="provid" id="provid" data-placeholder="省份" style="vertical-align:top;width: 120px;">
					...
					<td style="vertical-align:top;"> 
					 	<select class="chzn-select" name="cityid" id="cityid" data-placeholder="城市" style="vertical-align:top;width: 120px;">
					...
					<td style="vertical-align:top;"> 
					 	<select class="chzn-select" name="districtid" id="districtid" data-placeholder="路街" style="vertical-align:top;width: 120px;">
							<option value="-1" selected="selected">--区县--</option>
							<c:forEach items="${districtList}" var="obj" varStatus="vs">
								<option value="${obj.id }" <c:if test="${obj.id==pd.districtid }">selected</c:if>>${obj.name }</option>
							</c:forEach>
					  	</select>
					</td>
					<td>
						<input autocomplete="off" type="text" name="name" id="name" value="${pd.name }" placeholder="路街名称" />
					</td>
					<td style="vertical-align:top;"> 
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<c:choose>
					<c:when test="${not empty roadList}">
						<c:forEach items="${roadList}" var="obj" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;">${vs.index+1}</td>
								<td class='center' style="width: 30px;">${obj.id}</td>
								<td><b>${obj.name }</b></td>
								<td><b>${obj.districtName }</b></td>
								...
								<td class="center" style="width: 120px;">
									<a title="编辑" class='btn btn-mini btn-info updateRoad'><i class='icon-edit'></i></a>
								</td>
							</tr>
						</c:forEach>
					</c:when>
		...
		<table style="width:100%;">
			<tr>
				<td style="vertical-align:top;"><a class="btn btn-small btn-success" id="roadAdd">新增路街</a></td>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" style="width:538px;height:308px;display: none;">
	<div class="widget-box" style="width:540px;margin-left: -1px;margin-top: -2px;">
		...
		<div class="widget-body">
		 <div class="widget-main">
		 	<div class="row-fluid">
				<label for="form-field-12">路街ID</label>
			...
			<div class="row-fluid">
				...
				<div id="upDistrictID"><input type="text" name="modelDistrictID" id="modelDistrictID" value=""/></div>
				<div id="addDistrictID"><select class="chzn-select" name="modelDistrictID" id="modelDistrictID" data-placeholder="区县" style="vertical-align:top;width: 120px;">
					<c:forEach items="${districtList}" var="obj" varStatus="vs">
						<option value="${obj.id }" <c:if test="${obj.name==pd.modelDistrictV }">selected</c:if>>${obj.name }</option>
					</c:forEach>
			  	</select></div>
			</div>
			<hr />
			<div class="row-fluid">
				<label for="form-field-12">路街名称</label>
				<input type="text" name="modelName" id="modelName" value="" placeholder="解放北路"/> 
			</div>
			<hr />
			<div class="row-fluid">
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		$("#roadAdd").click(function(){
			...
			document.getElementById("addDistrictID").style.display="block";//显示
			document.getElementById("upDistrictID").style.display="none";//隐藏
		...
		$("#saveBtn").click(function(){
			...
			var url = "<%=basePath%>road/insertRoad.do";
			$("#Form").attr("action", url );
			$("#Form").submit();
		});
		...
		//修改路街信息
		$(".updateRoad").click(function(){
			var s=$(this).parent().parent().find("td");
			
			$("#modelId").val(s.eq(1).text());
			$("#modelName").val(s.eq(2).text());
			$("#modelProvID").val(s.eq(5).text());
			$("#modelCityID").val(s.eq(4).text());
			$("#modelDistrictID").val(s.eq(3).text());
			if( s.eq(6).text().indexOf("开启")>0  ){
			...
			document.getElementById("upDistrictID").style.display="block";//显示
			document.getElementById("addDistrictID").style.display="none";//隐藏
			...
			$("#modelDistrictID").prop("disabled",true);
			$("#saveDiv").hide();
			$("#updateDiv").show();
			$('#myModal').modal();
		...
		$("#updateBtn").click(function(){
			...
			var modelDistrictID = $("#modelDistrictID").val().trim();
			var isvalidV=$('input:radio:checked').val();
			if( modelId == '' ){
				$("#modelId").val("");
				toastr.warning('请输入数据');
			...
			$.ajax({
				...
				data:{
					...
					"modelDistrictID":modelDistrictID,
				},
				success:function(flag){
					if(flag==1){
			    	   document.getElementById("modelName").value="";
			    	   toastr.error('路街名称已经存在');
			    	   return ;
			       	}else if(flag==2){
			    	   location.href="./road/roadList.do";
			       	}  
			    }
			});
