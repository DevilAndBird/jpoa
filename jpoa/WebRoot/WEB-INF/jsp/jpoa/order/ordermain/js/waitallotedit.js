...
function fillwaitallotorderinfo() {
	...
	$.ajax({
		url : "./orderinfo/findOrderAllocinfo",
		data : {
			orderid : orderid
		},
		dataType : "json",
		...
		success : function(data) {
			...
			$.each(json, function (i, item) {
				...
				// 加载出发地目的地;
				parent.loadordersrcanddest(srcgps.lng, srcgps.lat, destgps.lng, destgps.lat);
			...
			$.each(json, function (i, item) {
				actionlist = actionlist + "<tr>" +
                  ...
                  actionlist = actionlist +"</td>" +
											"<td class='center'>" + json[i].arrivedtime + "</td>" +
											"<td class='center'>";
...
// 图片加载
$('.imgorderallotpath').attr('src', IMG_URL_PATH.ROUTE_32px_URL.value);
...
// 查看分配路径
function orderallotpath() {
	parent.lookpath($('#orderid').val().trim());
}
