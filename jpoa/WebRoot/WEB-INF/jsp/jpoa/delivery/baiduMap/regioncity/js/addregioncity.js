...
function loadDrawRegionCityColor() {
	$("#seleDrawColor").empty();
	for(i in DARWCOLOR) {
		$("#seleDrawColor").append("<option value='"+DARWCOLOR[i].value+"'>"+DARWCOLOR[i].name+"</option>");
	}
...
// 保存
$("#btnSaveRegionCityGps").click(function() {
	...
	$.ajax({
		url : "regionManage/saveRegionCityGps",
		data : {
			provid : provid,
			cityid : cityid,
			gps : gps,
		...
		success : function(data) {
			layer.alert(data, {
				...
				yes : function() {
					...
					if(descs[0] == "SUCCESS") {
						location.href = "./regionManage/addregioncity.do";
					}
				}
			});
		},
