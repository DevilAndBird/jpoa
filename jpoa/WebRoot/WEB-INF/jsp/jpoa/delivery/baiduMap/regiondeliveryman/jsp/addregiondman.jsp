...
<html>
	...
	<body>
		<div id="addTool">
			<table>
				<tr >
					...
					<td>
						&nbsp;&nbsp;
						<button class="btn btn-mini btn-light" onclick="saveRegionDmanPoint()" title="保存所画区域" data-toggle="modal" data-target="#myModal">
         ...
         <div class="modal-dialog">
             <div class="modal-content">
                 <div class="modal-header">
                     ...
                     <h4 id="h4DeliveryMan" class="modal-title">新增快递员区域划分</h4>
                 </div>
                 <div class="modal-body">
                         
						<div>
							<input type="text" id="regionname" placeholder="请输入区域名"/> 
						</div>
	                     <div>
                          	<select name="provid" id="provid" data-placeholder="省份" style="vertical-align:top;width: 120px;" disabled="disabled">
								...
								<c:forEach items="${provList}" var="obj" varStatus="vs">
									<option value="${obj.id}" <c:if test="${obj.id==pd.loginperson_provid}">selected</c:if>>${obj.name }</option>
								</c:forEach>
						  	</select>
						    <select name="cityid" id="cityid" data-placeholder="" style="vertical-align:top;width: 120px;" disabled="disabled">
								<option value="${pd.loginperson_cityid}" selected="selected">${pd.cityname}</option>
						  	</select>
						  </div>
						<div>
             ...
             <div class="modal-footer">
                 ...
                 <button id="btnSaveRegionDmanGps" type="button" class="btn btn-primary">保存</button>
             </div>
         </div>
         </div>
		...
		<script type="text/javascript" src="${ctx}/jpoa/delivery/baiduMap/regiondeliveryman/js/addregiondman.js?r=<%=new Random().nextInt() %>"></script>
