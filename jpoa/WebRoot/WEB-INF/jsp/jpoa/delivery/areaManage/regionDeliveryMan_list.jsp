...
<body>
	<div class="container-fluid" id="main-container">
	 <div id="page-content" class="clearfix">
	   <div class="row-fluid">
			<form action="regionManage/regionDeliveryManList" method="post" name="Form" id="Form">
			   <table>
					<tr>
						...
						<td style="vertical-align:top;"> 
						 	<select name="transitid" id="seleTransitid" data-placeholder="集散中心" style="vertical-align:top;width: 120px;">
						...
						<td style="vertical-align:top;"> 
						 	<select name="deliverymanid" id="seleDeliverymanid" data-placeholder="快递员" style="vertical-align:top;width: 120px;">
								<option value="-1" selected="selected">--快递员--</option>
								<c:forEach items="${userDeliveryManList}" var="obj" varStatus="vs">
									<option value="${obj.id}" <c:if test="${obj.id==pd.deliverymanid}">selected</c:if>>${obj.name}</option>
								</c:forEach>
						  	</select>
						</td>
					    ...
					    <td style="vertical-align:top;">
					    	&nbsp;&nbsp;&nbsp;&nbsp;
					    	<button class="btn btn-mini btn-light" title="工作区域添加" onclick="addRegionDman()" >
						   	 	<i id="nav-search-icon" class="icon-download-alt">工作区域添加</i>
						    </button>
					    </td>
					    <td style="vertical-align:top;">
					   		 <button class="btn btn-mini btn-light" title="工作区域查看" onclick="viewRegionDman()">
				...
				<table id="table_report" class="table table-striped table-bordered table-hover">
					...
					<!-- 开始循环 -->	
					<c:choose>
						<c:when test="${not empty regionDeliveryManList}">
							<c:forEach items="${regionDeliveryManList}" var="regionDeliveryMan" varStatus="vs">
								<tr>
									<td class="center">${vs.index+1}</td>
									<td class="center">${regionDeliveryMan.provDesc}</td>
									<td class="center">${regionDeliveryMan.cityDesc}</td>
									<td class="center">${regionDeliveryMan.regionname}</td>
								    <td class="center">
								    	<button title="编辑快递员区域区域" class="btn btn-link" onclick="editRegionDman(this);" value="${regionDeliveryMan.id}" >编辑</button>
								    </td>
								</tr>
							</c:forEach>
						</c:when>
		...
		<script type="text/javascript">
			...
			/* 添加工作区域 */
			function addRegionDman() {
		    	 var url = "<%=basePath%>regionManage/addRegionDman.do";
		    	 $("#Form").attr("action", url);
		    	 $("#Form").submit();
			}
			
			/* 编辑工作区域 */
			function editRegionDman(param) {
				 var id = $(param).val();
		    	 var url = "<%=basePath%>regionManage/editRegionDman.do?id=" + id;
		    	 $("#Form").attr("action", url);
		    	 $("#Form").submit();
			}
			
			/* 查看工作区域 */
			function viewRegionDman() {
		    	 var url = "<%=basePath%>regionManage/viewRegionDman.do";
		    	 $("#Form").attr("action", url);
		    	 $("#Form").submit();
			}
