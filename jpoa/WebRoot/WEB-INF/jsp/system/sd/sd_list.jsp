...
<div id="page-content" class="clearfix">				
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="sheetDetail/sdlistPage" method="post" name="userForm" id="userForm">
			<table>
				<tr>
					<td>
					...
					<c:when test="${not empty userList}">
						<c:forEach items="${userList}" var="user" varStatus="vs">
							...
							<tr>
								...
								<td>${user.typename}</td>
								<td>${user.size }</td>
								<td>${user.isnull==1?"不为空":"可为空" }</td>
								<td>${user.itemcode }</td>
								<td>${user.notes }</td>
								<td style="width: 60px;">
									<div class='hidden-phone visible-desktop btn-group'>
												 <a class='btn btn-mini btn-danger' onclick="delUser('${user.id }','${fidtemp}','${user.label }');">删除<i class='icon-trash'></i></a>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>sheetDetail/gosave.do?fidtemp='+${fidtemp};
			 diag.Width = 480
			 diag.Height = 500;
		...
		function editUser(user_id){
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '<%=basePath%>/sheetDetail/goupdate.do?id='+user_id;
			 diag.Width = 600;
			 diag.Height = 800;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		//删除
		function delUser(userId,fid,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>sheetDetail/deletesd.do?id="+userId+"&fidtemp=${fid}";
