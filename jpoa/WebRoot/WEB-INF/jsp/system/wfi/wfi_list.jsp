...
<div id="page-content" class="clearfix">				
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="wfi/wfilistPage" method="post" name="sheetForm" id="sheetForm">
			<table>
				<tr>
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="flowname" placeholder="这里输入关键词" />
					...
					<c:when test="${not empty userList}">
						<c:forEach items="${userList}" var="user" varStatus="vs">								
							<tr>
								...
								<td><a>${user.flowname }</a></td>
								<td>${user.notes }</td>
								<td style="width: 60px;">
									<div class='hidden-phone visible-desktop btn-group'>
										         <a class='btn btn-mini btn-danger' href="<%=request.getContextPath()%>/wfd/wfdlistPage?wfiid=${user.id}">设计步骤<i class='icon-trash'></i></a>
									</div>
								</td>
							</tr>
						</c:forEach>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增表单";
			 diag.URL = '<%=basePath%>wfi/gosave.do';
			 diag.Width = 230;
			 diag.Height = 200;
